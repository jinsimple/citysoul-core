...
if ( !class_exists( 'Radium_Theme_Importer' ) ) {
  ...
  class Radium_Theme_Importer {
    ...
    public function __construct() {
      ...
      $this->content_megamenu = $this->demo_files_path . 'content-megamenu.xml';
    ...
    public function process_imports() {
      ...
      if($_POST['current_process'] == 'posts_content') {
          if ( !empty( $this->content_demo ) && is_file( $this->content_demo ) ) {
            ...
            if( !empty($this->content_megamenu) && is_file($this->content_megamenu)){
                $this->import_megamenu_theme( $this->content_megamenu );     
            }
            if(function_exists('after_setting_page')) {
                after_setting_page();
            }
    ...
    public function set_demo_menu() {       
        if(!is_file($this->demo_files_path . 'setting-page.php')){           
            return;
        } 
    ...
    public function import_widgets( $data ) {
      ...
      foreach ( $data as $sidebar_id => $widgets ) {
        ...
        foreach ( $widgets as $widget_instance_id => $widget ) {
          ...
          // No failure
          if ( ! $fail ) {
            /* Dean edit - mass fix for mega menu (stupid fix but faster ~_~) */
            if(isset($widget->mega_menu_order)) {
                $widget->mega_menu_order = (array) $widget->mega_menu_order;
            }
            /* End Dean */
