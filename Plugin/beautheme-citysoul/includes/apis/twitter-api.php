<?php
function citysoul_latest_tweets_render( $count, $rts= true, $ats= true, $pop = 0, $screen_name='' ){
    try {
        ...
        foreach( $tweets as $tweet ){
            ...
            if( $final === $html ){
                ...
                $final =  '<li><div class="twitter-user"><a href="#" class="text-active"><i class="fa fa-twitter"></i>.'.$handle.'</a><a href="#" class="name-follow">@'.strtolower($handle).'</a> - <span class="time">'.$date.'</span></div>'.
                            '<div class="message text-more">
                                '.$html.'
                            </div></li>';
            }
            $rendered[] = $final;
...
function citysoul_follow_tweets( $count, $rts= true, $ats= true, $pop = 0, $screen_name='' ){
    try {
        if( ! function_exists('twitter_api_get') ){
            if (file_exists(BEAU_PLUGIN_DIR.'/libs/wp-twitter-api/twitter-api.php')) {
                require_once (BEAU_PLUGIN_DIR.'/libs/wp-twitter-api/twitter-api.php');
